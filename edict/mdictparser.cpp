...
QByteArray zlibDecompress(const char *bufptr, unsigned length) {
  ...
  inflateEnd(&zs);
  if (res != Z_STREAM_END) str.clear();
  return str;
}

enum EncryptedSection { EcryptedHeadWordHeader = 1, EcryptedHeadWordIndex = 2 };

static inline int u16StrSize(const ushort *unicode) {
  int size = 0;
  if (unicode) {
    while (unicode[size] != 0) size++;
  }
  return size;
}
...
size_t MdictParser::RecordIndex::bsearch(
    const vector<MdictParser::RecordIndex> &offsets, qint64 val) {
  if (offsets.size() == 0) return (size_t)(-1);
...
MdictParser::~MdictParser() {
  if (!file_.isNull()) file_->close();
}

bool MdictParser::checkAdler32(const char *buffer, unsigned int len,
                               quint32 checksum) {
...
bool MdictParser::parseCompressedBlock(qint64 compressedBlockSize,
  ...
  quint32 checksum =
      qFromBigEndian<quint32>((const uchar *)compressedBlockPtr + 4);
...
bool MdictParser::open(const QString &filename) {
  ...
  if (!readRecordBlockInfos()) return false;
  return true;
}

bool MdictParser::readNextHeadWordIndex(
  ...
  if (compressedSize < 8) return false;

  ScopedMemMap compressed(*file_, headWordPos_, compressedSize);
  if (!compressed.startAddress()) return false;
...
MdictParser::HeadWordIndex MdictParser::splitHeadWordBlock(
    QByteArray const &block) {
  HeadWordIndex index;
  ...
  while (p < end) {
    ...
    p += headWordBuf.size();
    QString headWord =
        toUtf16(encoding_, headWordBuf.constBegin(), headWordBuf.size());
...
bool MdictParser::readRecordBlock(MdictParser::HeadWordIndex &headWordIndex,
  ...
  for (HeadWordIndex::const_iterator i = headWordIndex.begin();
       i != headWordIndex.end(); i++) {
...
bool MdictParser::readHeader(QDataStream &in) {
  ...
  if (headerTextUtf16.size() != headerTextSize) return false;

  quint32 checksum;
  in.setByteOrder(QDataStream::LittleEndian);
  in >> checksum;
  if (!checkAdler32(headerTextUtf16.constData(), headerTextUtf16.size(),
                    checksum)) {
    qDebug("MDict: readHeader: checksum does not match");
    return false;
  }
  in.setByteOrder(QDataStream::BigEndian);
  ...
  if (headerAttributes.contains("StyleSheet")) {
    QString styleSheets =
        headerAttributes.namedItem("StyleSheet").toAttr().value();
    QStringList lines =
        styleSheets.split(QRegExp("[\r\n]"), QString::KeepEmptyParts);
    for (int i = 0; i < lines.size() - 3; i += 3) {
      styleSheets_[lines[i].toInt()] =
          pair<QString, QString>(lines[i + 1], lines[i + 2]);
    }
  }

  // before version 2.0, number is 4 bytes integer
  // version 2.0 and above uses 8 bytes
  version_ = headerAttributes.namedItem("GeneratedByEngineVersion")
                 .toAttr()
                 .value()
                 .toDouble();
  if (version_ < 2.0)
  ...
  QString description =
      headerAttributes.namedItem("Description").toAttr().value();
...
bool MdictParser::readHeadWordBlockInfos(QDataStream &in) {
  ...
  if (version_ >= 2.0) stream >> decompressedSize;
  ...
  if (headWordBlockInfo.size() != headWordBlockInfoSize_) return false;
  ...
  if (version_ >= 2.0) {
    ...
    QByteArray decompressed;
    if (!parseCompressedBlock(headWordBlockInfo.size(),
                              headWordBlockInfo.data(), decompressedSize,
                              decompressed))
      return false;
...
MdictParser::BlockInfoVector MdictParser::decodeHeadWordBlockInfo(
    QByteArray const &headWordBlockInfo) {
  ...
  while (!s.atEnd()) {
    ...
    headWordBlockInfos.push_back(
        BlockInfoVector::value_type(compressedSize, decompressedSize));
  }
